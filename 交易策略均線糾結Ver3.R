...
allstock$date<-as.Date(paste0(substr(allstock$date,1,4),"-",substr(allstock$date,5,6),"-",substr(allstock$date,7,8)))
...
for(i in idd$id ){  #此區iid為用sas重新篩選之股票
 ...
 #allstock[ which('1101' == allstock$id),]
  ...
  stopProfitRatio <- 0.8#此處為停利比率(可調整)
...
conclusion$tradeDayDiff <- conclusion$sellDate-conclusion$buyDate
avgDay<-mean(conclusion$tradeDayDiff, na.rm=T)

yearprofit <- (avgRet+1)^(252/as.numeric(avgDay))-1

sum(TotalCluData001$TradeVol)
